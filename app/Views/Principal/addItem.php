                    ...
                    <?php if(isset($_SESSION['Droit']) && $tabXlsFields[$_GET['XLS']]['profilMenu'][$_SESSION['Droit']]['ecriture'] == 1) { ?>
                        ...
                        <form method="post" enctype="multipart/form-data">
                            ...
                            foreach ($champs as $key => $v){
                                ...
                                if(isset($v['profilChamps'][$_SESSION['Droit']]['ecriture']) && $v['profilChamps'][$_SESSION['Droit']]['ecriture'] == 1 && $v['type_input'] != 'categorie' && $v['type_input'] != 'sous_categorie'){
                                        ...
                                        if(!empty($errors) && $errors != null){
                                            foreach($errors->errors as $error => $e):
                                                if($error != null && $key == $error){?>
                                                    <?php echo $form->inputWithIdAutocompPrincipal('inputItem'.$v['bdd_table_t'], $v['bdd_table_t'], $v['nom'].' '. $e, $v['bdd_table'], $v['bdd_value'], $_GET['XLS'], $v['bdd_table_t'], $v['taille_min'], $v['taille_max'], $key);?>
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                                            ...
                                            <?php echo $form->inputWithIdAutocompPrincipal('inputItem'.$v['bdd_table_t'], $v['bdd_table_t'], $v['nom'].' :', $v['bdd_table'], $v['bdd_value'], $_GET['XLS'], $v['bdd_table_t'], $v['taille_min'], $v['taille_max'], $key);?>
                                            <?php if (isset($_POST[$v['bdd_value']])){ ?>
                                                <fieldset disabled>
                                                    <div class="form-group champHidAutoComp">
                                                        <label>Label :</label>
